...
// Populates the database with the SAME placeholder data that was in
// src/components/portfolio/data.ts, so switching frontend -> API is a
// transparent swap. TODO: [REPLACE ALL OF THIS WITH YOUR REAL DATA]
// Easiest way to replace it for good: edit this file once with your real
// content and re-run `npm run seed`, OR build out admin UI screens that
// call the PUT/POST/DELETE endpoints.
...
const profile = {
  ...
  roles: ["Full Stack Developer", "Problem Solver", "Open Source Contributor"], // [REPLACE]
  tagline:
    "I design and ship resilient web, mobile, and backend systems — from idea to scale.", // [REPLACE]
  email: "hello@alexdoe.dev", // [REPLACE]
  location: "Bengaluru, India", // [REPLACE]
  availability: "Freelance · Full-time · Collaborations", // [REPLACE]
...
const projects = [
  {
    ...
    description: "Realtime product analytics dashboard with sub-second query latency on 1B+ events.",
    category: "Web",
  ...
  },
  {
    ...
    image: "https://images.unsplash.com/photo-1639762681485-074b7f938ba0?auto=format&fit=crop&w=1200&q=70",
    order: 6,
  },
];
...
// Skills are stored as GROUPS (one doc per category) to match
// `skills: SkillGroup[]` in data.ts exactly.
const skillGroups = [
  {
    category: "Frontend",
    order: 1,
...
const seed = async () => {
  try {
    ...
    await ResearchPaper.insertMany(researchPapers);
    await Skill.insertMany(skillGroups);
